...
import numpy as np
from utils import  prepare_cifar10_model, get_cifar10_data
...
def main(args): 
    ...
    model = prepare_cifar10_model(args.device)


    train_loader, test_loader = get_cifar10_data(transform=torchvision.transforms.ToTensor(), batch_size=12)
    ...
    for x, y in test_loader:
        ...
        if ju % 10 == 0:
            plt.figure()
            plt.title(f'GT = {classes[Y[0].item()]} Pred = {classes[pred_c[0]]}')
            plt.imshow(np.transpose(q.to('cpu').detach().numpy(),(1, 2, 0)))
            plt.savefig(f'cifar_after{ju}.png')

            plt.figure()
            plt.title(f'GT = {classes[Y[0].item()]} Pred = {classes[pred[0]]}')
            plt.imshow(np.transpose(q2.to('cpu').detach().numpy(),(1, 2, 0)))
            plt.savefig(f'cifar10_before{ju}.png')
